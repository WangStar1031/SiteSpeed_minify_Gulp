		 ...
		 <input type="hidden" id="contact_email" value="" name="contact_email" > 
			...
			<div class="row content-wrapper">
				<div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 card-select">
					...
					<!-- This is hidden for some purpose -->
					<!-- This is hidden for some purpose -->
					<!-- This is hidden for some purpose -->
					<!-- This is hidden for some purpose -->
					<!-- This is hidden for some purpose -->
...
$('#button-confirmm').on('click', function() {
 // alert(123);
	...
	$.ajax({
        url: '<?php echo BASE; ?>index.php?route=cloverAPI/clover/single_page_checkout_save_order',
        type: 'post',
...
if(json['paythere_issue']){
		bootbox.alert("<span class='custom_bootbox_error'>Oh-oh! "+json['paythere_issue']+"</span>", function() {
		  location.href = "<?php echo BASE; ?>index.php?route=cloverAPI/clover/checkout_form";
		});
}
if(json['paymentId'] && json['result']=="APPROVED" || json['result']=="SUCCESS"){
	location.href = "<?php echo BASE; ?>index.php?route=cloverAPI/clover/success_order&order_id="+json['order_id']+"&merchant_id="+json['merchant_id']+"";
	return;
}          if (json['redirect']) {
				...
				if (json['error']['error_main']) {
						$("#cim_payment_option2").after('<div class="text-danger">' + json['error']['error_main']['message'] + '</div>');
				}
				for (i in json['error_card']) {
					// var element = $('#'+i.replace('_', '-')+'_check');
					$("#cim_payment_option2").after('<div class="text-danger">' + json['error_card'][i] + '</div>');
				}
				...
				if(json['error']['zone_id']){
					$('select[name=\'zone_id\']').after('<div class="text-danger">' + json['error']['zone_id'] + '</div>');
				}		
