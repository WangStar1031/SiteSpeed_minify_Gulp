...
<body style="font-family: 'Gill Sans','Gill Sans MT','Myriad Pro','DejaVu Sans Condensed',Helvetica,Arial,sans-serif;">
   <div class="order_outer" style=" max-width: 580px;background:#778b8b;margin:0 auto;padding: 3px; overflow: hidden;">
			...
			<div class="order_content_outer" style="float:left;width:80%;background:#fff;padding: 3%; margin: 0 7%; border-radius: 20px 20px 0 0;">
								   ...
								   <p style="margin:0;font-weight:normal; font-size: 16px;color: #000;"><?php echo $customerFirstName.' '.$customerLastName; ?></p>
								 <?php if($products[0]['delivery_address'] && trim($products[0]['delivery_address']) != '' ) { ?>  
								   <p style="margin:0;font-weight:normal; font-size: 16px;color: #000;"><?php echo $products[0]['delivery_address']; ?></p>
								  <?php } ?>
								   <p style="margin:0;font-weight:normal; font-size: 16px;color: #000;"><?php echo $order_delivery_method.' @ '.$combinedOrderTime; ?></p>
					 ...
					 <ul style="float:left;width:100%;padding:0;list-style:none; border-bottom: 2px solid #8d8d8d; padding-bottom: 15px;">
								...
								if( !isset($option['simple_option']) ){
									...
									ksort($option) ;
											...
											<div style="font-size: 13px; clear:both;">- <?php echo $option['name'];?></div>
											<?php
													$tempArrOption[] = $option['name'] ;
													$j 	=	1;
													$i 	=   0;	
											} }?>								
											<div class="subitems bold" style="margin-top: 5px;"></div>
											<div style="font-size: 13px; clear:both; border-bottom: 1px solid;width: 41px;">
													<?php if( $type == '2' ){?>
														 RIGHT 
											...
											?>
												  ...
												  <li style="float:left;width:100%;padding:0;list-style:none;margin-bottom:0px;margin-left:0;">
												   <div style="margin-bottom: 0px;  width: 100%;float:left;">
													...
													<div style="float:left;font-size: 12px;max-width:80%;color:#666;"> <?php if( $option['option_qty'] !=0) { echo $option['option_qty']. ' x ';} else { }  ?><?php echo $option['value']; ?> </div>
													...
													<?php if( $option['topping_option'] == 2 ){?>
														<small><span style="padding-right:5px; font-size:11px; clear:both;">2x</span></small>
													<?php }?>
								...
								?>
									  ...
									  <?php  
											$i = 1; foreach ($product['option'] as $option) { ?>
									  <li style="float:left;width:100%;padding:0;list-style:none;margin-bottom:0px;margin-left:0;">
									   <div style="margin-bottom: 0px;  width: 100%;float:left;">
											<?php if(!in_array($option['name'],$tempArrOption)){?>
											<div style="font-size: 13px; clear:both;">- <?php echo $option['name'];?></div>
